...
<div class="box">
  <div class="box box--right">
    ...
    <div class="box__description">
        ...
        SELECT * from uzytkownicy WHERE tozsamosc = 'Uczen' order by Nazwisko ASC;
        </sql:query>
        ...
        <c:forEach var="wiersz" items="${wynik.rows}">
            <tr>
                <td><c:out value="${licznik}"/></td>
                <td><c:out value="${wiersz.nazwisko}"/></td>
                <td><c:out value="${wiersz.Imie}"/></td>
                <td><form method="post" action="modyfikuj_uczniowie.jsp"><input type="hidden" name="edytuj_ktory" value="${wiersz.ID_uzytkownicy}" /><button name="edytuj">Modyfikuj</button></form></td>
                <td><form method="post" action="usun_uczniowie.jsp"><input type="hidden" name="usun_ktory" value="${wiersz.ID_uzytkownicy}" /><button name="usun">Usuń</button></form></td>             
            </tr>
          
            <c:set var="licznik" value="${licznik + 1}" scope="page"/>
        </c:forEach>
