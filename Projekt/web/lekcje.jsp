    ...
    <div class="box__description">
        ...
        <c:if test="${pageContext.request.method=='POST'}">
            ...
            <c:when test="${param.przedmiot == wybrany_przedmiot}">
                <sql:query dataSource="${Polaczenie}" var="lekcje">
                SELECT * from lekcje JOIN przedmioty on lekcje.ID_przedmiotu = przedmioty.ID_przedmioty WHERE Nazwa = ?;
                ...
                <table>
                    ...
                    <c:forEach var="wiersz" items="${lekcje.rows}">
                    <tr>
                        <td><c:out value="${wiersz.nr_lekcji}"/></td>
                        <td><c:out value="${wiersz.Temat}"/></td>
                        <td><c:out value="${wiersz.Data_zajec}"/></td>
                        <td><form method="post" action="obecnosci.jsp"><input type="hidden" name="ktora_lekcja" value="${wiersz.ID_lekcje}"/><button name="obec">Sprawdź obecność</button></form></td>
                    </tr>
                    </c:forEach>
